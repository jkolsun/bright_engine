...
export default function ClawdBotTab() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[40vh] bg-black rounded-lg">
  ...
  return (
    <div className="bg-black text-white p-4 pb-20 md:p-8 max-w-full w-full rounded-lg">
      {/* Header */}
      <div className="mb-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <div className="bg-gray-900 p-3 rounded border border-gray-800">
            <p className="text-xs text-gray-400 dark:text-gray-500">📱 Texts</p>
            <p className="text-2xl font-bold text-blue-400">{stats.textsSent}</p>
          </div>
          <div className="bg-gray-900 p-3 rounded border border-gray-800">
            <p className="text-xs text-gray-400 dark:text-gray-500">🔗 Previews</p>
            <p className="text-2xl font-bold">{stats.previewsGenerated}</p>
          </div>
          <div className="bg-gray-900 p-3 rounded border border-gray-800">
            <p className="text-xs text-gray-400 dark:text-gray-500">💰 Upsells</p>
        ...
        {expandedSections.queues && queues && (
          <div className="border-t border-gray-800 p-4 space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-400 dark:text-gray-500">Nurture active</span>
              <span className="font-bold text-cyan-400">{queues.nurture_active}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400 dark:text-gray-500">Preview urgency</span>
              <span className="font-bold text-yellow-400">{queues.preview_urgency}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400 dark:text-gray-500">Upsells pending</span>
              <span className="font-bold text-green-400">{queues.upsell_pending}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400 dark:text-gray-500">Referrals pending</span>
              <span className="font-bold text-purple-400">{queues.referral_pending}</span>
            </div>
            {queues.total_hot > 0 && (
              <div className="border-t border-gray-700 dark:border-slate-600 pt-3 mt-3">
                <p className="text-xs text-gray-400 dark:text-gray-500 mb-2">🔥 Hot leads ({queues.total_hot})</p>
                <div className="space-y-1">
                  {queues.leads.slice(0, 3).map(lead => (
                    <div key={lead.id} className="text-xs text-gray-300 dark:text-gray-400 truncate">
                      {lead.companyName}
                    </div>
                  ))}
                </div>
      ...
      <div className="bg-gray-900 rounded border border-gray-800 mb-4 overflow-hidden">
        ...
        {expandedSections.stats && stats && (
          <div className="border-t border-gray-800 p-4 space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-gray-400 dark:text-gray-500">Texts received</span>
              <span className="text-blue-400">{stats.textsReceived}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400 dark:text-gray-500">Alerts sent</span>
              <span className="text-orange-400">{stats.alertsSent}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400 dark:text-gray-500">Referrals asked</span>
              <span className="text-purple-400">{stats.referralsAsked}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400 dark:text-gray-500">Errors</span>
      ...
      <div className="bg-gray-900 rounded border border-gray-800 overflow-hidden">
        ...
        <div className="divide-y divide-gray-800 max-h-96 overflow-y-auto">
          {activity.length === 0 ? (
            <div className="p-4 text-xs text-gray-500 dark:text-gray-400">No activity yet</div>
          ) : (
              ...
              <div key={item.id} className="p-3 hover:bg-gray-800 active:bg-gray-700 text-xs">
                <div className="flex gap-2 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-gray-300 dark:text-gray-400 text-xs truncate">{item.description}</p>
                  ...
                  <div className="text-gray-500 dark:text-gray-400 text-xs flex-shrink-0 whitespace-nowrap">
