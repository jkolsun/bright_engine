...
export default function EmailPreviewPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)]">
      {/* ── LEFT SIDEBAR ──────────────────────────────────── */}
      <div className="w-[300px] border-r border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex flex-col overflow-hidden">
        {/* Header */}
        <div className="p-4 border-b border-gray-200 dark:border-slate-700">
          <div className="flex items-center gap-2 mb-3">
            <Send size={18} className="text-blue-600" />
            <h2 className="font-semibold text-gray-900 dark:text-gray-100">Test Email</h2>
          </div>

          {/* Send from */}
          <div className="mb-3">
            <label className="text-xs font-medium text-gray-500 dark:text-gray-400 block mb-1">Send from:</label>
            <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 dark:bg-slate-800/50 rounded-lg border border-gray-200 dark:border-slate-700 text-sm text-gray-700 dark:text-gray-300">
          ...
          <div className="relative">
            <label className="text-xs font-medium text-gray-500 dark:text-gray-400 block mb-1">Load data for lead:</label>
            <div
              className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-700 cursor-pointer hover:border-blue-300 transition-colors"
            ...
            >
              {selectedLead ? (
                ...
                <span className="text-sm text-gray-400 dark:text-gray-500 flex-1">Select a lead...</span>
              )}
              {dropdownOpen ? <ChevronUp size={14} className="text-gray-400 dark:text-gray-500" /> : <ChevronDown size={14} className="text-gray-400 dark:text-gray-500" />}
            </div>

            {dropdownOpen && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg z-50 max-h-[300px] overflow-hidden">
                <div className="p-2 border-b border-gray-100 dark:border-slate-800">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-1.5 text-sm border border-gray-200 dark:border-slate-700 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 dark:bg-slate-800 dark:text-gray-200"
                      autoFocus
                    />
                  </div>
                </div>
                <div className="max-h-[240px] overflow-y-auto">
                  {filteredLeads.map((lead) => (
                    <button
                      ...
                      className="w-full text-left px-3 py-2 hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-colors border-b border-gray-50 dark:border-slate-800 last:border-0"
                    >
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{lead.firstName} {lead.lastName}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">{lead.companyName} {lead.email ? `· ${lead.email}` : ''}</div>
                    </button>
                  ))}
                  {filteredLeads.length === 0 && (
                    <div className="px-3 py-4 text-sm text-gray-400 dark:text-gray-500 text-center">No leads found</div>
                  )}
                </div>
              </div>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">Variables</span>
          </div>
          <div className="border-t border-dashed border-gray-300 dark:border-slate-600 mb-3" />

          {selectedLead ? (
            <div className="space-y-4">
              {Object.entries(mergeVars).map(([key, value]) => (
                <div key={key}>
                  <label className="text-xs text-gray-400 dark:text-gray-500 block mb-0.5">{key}</label>
      ...
      {/* ── MAIN CONTENT ──────────────────────────────────── */}
      <div className="flex-1 bg-gray-50 dark:bg-slate-800/50 flex flex-col overflow-hidden">
        {/* Top bar */}
        <div className="p-4 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Eye size={20} className="text-gray-500 dark:text-gray-400" />
            <h1 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Email Preview</h1>
            {selectedLead && (
        ...
        {!selectedLead ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <Mail size={48} className="text-gray-300 dark:text-gray-600 mx-auto mb-4" />
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Select a lead to preview</h3>
              <p className="text-gray-500 dark:text-gray-400 text-sm">Choose a lead from the sidebar to see the email sequence with their data filled in.</p>
            </div>
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto p-6">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                    activeStep === idx
                      ...
                      : 'bg-white dark:bg-slate-900 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700 border border-gray-200 dark:border-slate-700'
                  }`}
                >
                  Email {email.step}
            ...
            <Card className="max-w-3xl">
              ...
              <div className="p-5 border-b border-gray-100 dark:border-slate-800 space-y-3">
                <div className="flex items-center gap-3">
                  <span className="text-sm text-gray-400 dark:text-gray-500 w-16">Send to:</span>
                ...
                <div className="flex items-center gap-3">
                  <span className="text-sm text-gray-400 dark:text-gray-500 w-16">Subject:</span>
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{renderedSubject}</span>
                </div>
              </div>
            ...
            <Card className="max-w-3xl mt-4 p-4">
              <div className="flex items-center gap-6 text-sm text-gray-600 dark:text-gray-400">
                ...
                {selectedLead.phone && (
                  <div className="flex items-center gap-1.5">
                    <PhoneIcon size={14} className="text-gray-400 dark:text-gray-500" />
                    {selectedLead.phone}
                  </div>
                )}
              </div>
