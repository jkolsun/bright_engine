...
export default function PartTimeLayout({
  ...
  return (
    <ThemeProvider>
      <PartTimeLayoutInner>{children}</PartTimeLayoutInner>
    </ThemeProvider>
  )
}

function PartTimeLayoutInner({
  children,
}: {
  children: React.ReactNode
}) {
  ...
  return (
    <div className={`flex h-screen ${resolvedTheme === 'dark' ? 'dark bg-background' : ''}`} style={resolvedTheme === 'dark' ? undefined : { background: 'linear-gradient(135deg, #f8fafc 0%, #f0fdfa 40%, #ecfdf5 70%, #f0f9ff 100%)' }}>
