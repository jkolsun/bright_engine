...
export async function POST(
  ...
  try {
    ...
    const systemPrompt = `You are a world-class web designer and front-end developer with 15+ years of experience building high-converting business websites. You have deep expertise in HTML, Tailwind CSS, typography, color theory, layout composition, and conversion optimization. You receive a complete static HTML document and a plain-English modification instruction. Apply the requested changes with the precision and taste of a senior design professional.
...
The site belongs to: ${lead.companyName}`
    ...
    for (const edit of history) {
      ...
      messages.push({
        role: 'assistant',
        content: JSON.stringify({ changes: [], summary: edit.summary }),
      })
    }
    ...
    if (response.stop_reason === 'max_tokens') {
      ...
      return NextResponse.json(
        { error: 'AI response was too long and got cut off. Try a simpler instruction.' },
        { status: 500 }
      )
    }
    ...
    try {
      ...
      if (parsed.changes && Array.isArray(parsed.changes)) {
        ...
        for (const change of parsed.changes) {
          ...
          // Strategy 3: Trimmed line match — handle cases where Claude
          // grabs a snippet that spans line boundaries differently
          ...
          // Strategy 4: Substring match — if the search is a class attribute change,
          // try finding just the class="..." portion
        ...
        if (appliedCount === 0 && parsed.changes.length > 0) {
          ...
          return NextResponse.json(
            { error: `Could not apply changes — the AI's search strings didn't match the HTML. Try rephrasing: "${instruction}"` },
            { status: 500 }
          )
        }
      ...
      if (rawText.includes('<html') || rawText.includes('<!DOCTYPE')) {
        // Claude returned full HTML — use it directly
        ...
        if (fullHtml.includes('<html') || fullHtml.includes('<!DOCTYPE')) {
          ...
          return NextResponse.json(
            { error: 'AI returned invalid response. Try rephrasing your instruction.' },
            { status: 500 }
          )
        }
      } else {
        ...
        return NextResponse.json(
          { error: 'AI returned an unexpected format. Try a simpler instruction.' },
          { status: 500 }
        )
      }
    }
    ...
    if (message.includes('timeout') || message.includes('TIMEOUT')) {
      return NextResponse.json(
        { error: 'The AI took too long to respond. Try a simpler edit instruction.' },
        { status: 504 }
      )
    }
    ...
    if (message.includes('invalid_api_key') || message.includes('authentication')) {
      return NextResponse.json(
        { error: 'AI service authentication error. Contact admin.' },
        { status: 500 }
      )
    }
...
function trimmedLineReplace(html: string, search: string, replace: string): string | null {
  ...
  // Find the actual end: include any remaining characters of the last matched token
  let actualEnd = endPos + 1
  while (actualEnd < html.length && !/\s/.test(html[actualEnd]) && normalizedSearch.endsWith(html.slice(posMap[endIdx], actualEnd + 1).replace(/\s+/g, ' ').trim().slice(-1))) {
    actualEnd++
  }
  // Simpler: just go to end of the last non-whitespace run
  actualEnd = endPos + 1
  // If the normalized search ends with a non-space, extend to cover the full token
