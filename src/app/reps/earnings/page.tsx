...
import { DollarSign, TrendingUp, Calendar, Award, Clock, CheckCircle, AlertCircle, XCircle, Target, Send, Filter } from 'lucide-react'
  ...
  const loadEarnings = async () => {
    try {
      ...
      const commissions: Commission[] = (commData.commissions || []).map((c: Commission) => {
        // Parse deal amount and rate from notes JSON
        if (c.notes) {
          try {
      ...
      // Total earned = APPROVED + PAID only (NOT pending)
      const totalEarned = commissions
        .filter(c => c.status === 'APPROVED' || c.status === 'PAID')
        .reduce((sum, c) => sum + (c.amount || 0), 0)

      // This month (APPROVED + PAID only)
