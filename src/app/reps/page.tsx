      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <p className="text-gray-500 dark:text-gray-400 text-sm">{todayDate}</p>
            {lastUpdated && (
        ...
        <div className="flex gap-3">
          <Link href="/reps/dialer?mode=single">
            <Button variant="outline" className="gap-2 rounded-xl border-gray-200 dark:border-slate-700 hover:border-teal-300 hover:bg-teal-50/50 transition-all">
              <PhoneCall size={18} className="text-teal-600 dark:text-teal-400" /> Single Dial
            </Button>
          </Link>
      ...
      <Card className="p-6 rounded-2xl border-0 shadow-medium bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
        ...
        <div className="grid grid-cols-3 gap-8 mb-5">
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium uppercase tracking-wide">Dials</p>
          </div>
          <div>
            <div className="flex items-baseline gap-2">
          ...
          <div>
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-bold text-gray-900 dark:text-gray-100">{todayStats.wantsToMoveForward}</span>
              <span className="text-sm text-gray-400 dark:text-gray-500 font-medium">/ {dailyTarget.closes}</span>
            </div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium uppercase tracking-wide">Move Forward</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 mt-3">
          {[
            { key: 'wantsToMoveForward' as const, label: 'Move Fwd', bg: 'bg-green-100 dark:bg-green-900/40', text: 'text-green-700 dark:text-green-400' },
            { key: 'callbacks' as const, label: 'Callback', bg: 'bg-teal-100', text: 'text-teal-700 dark:text-teal-400' },
            { key: 'interestedVerbal' as const, label: 'Verbal Int', bg: 'bg-green-50 dark:bg-green-950/30', text: 'text-green-600' },
            { key: 'wantsChanges' as const, label: 'Changes', bg: 'bg-blue-100 dark:bg-blue-900/40', text: 'text-blue-700 dark:text-blue-400' },
            { key: 'willLookLater' as const, label: 'Look Later', bg: 'bg-amber-100 dark:bg-amber-900/40', text: 'text-amber-700 dark:text-amber-400' },
            { key: 'notInterested' as const, label: 'Not Int', bg: 'bg-gray-100 dark:bg-slate-800', text: 'text-gray-600 dark:text-gray-400' },
            { key: 'voicemails' as const, label: 'VM', bg: 'bg-gray-100 dark:bg-slate-800', text: 'text-gray-500 dark:text-gray-400' },
            { key: 'noAnswers' as const, label: 'No Ans', bg: 'bg-gray-50 dark:bg-slate-800/50', text: 'text-gray-500 dark:text-gray-400' },
            { key: 'wrongNumbers' as const, label: 'Wrong #', bg: 'bg-red-50 dark:bg-red-950/30', text: 'text-red-600' },
            { key: 'disconnected' as const, label: 'Disconn', bg: 'bg-red-50 dark:bg-red-950/30', text: 'text-red-600' },
            { key: 'dnc' as const, label: 'DNC', bg: 'bg-red-100 dark:bg-red-900/40', text: 'text-red-700 dark:text-red-400' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6 rounded-2xl border-0 shadow-medium bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
          ...
          <div className="space-y-2.5">
            ...
            {!queueSummary && (
              <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-4">Loading queue...</p>
            )}
            {queueSummary && (
                        ...
                        {lead.previewUrl && (
                          <>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="text-teal-600 dark:text-teal-400 hover:text-teal-700 hover:bg-teal-50 rounded-lg"
      ...
      <Card className="p-6 rounded-2xl border-0 shadow-medium bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <Link href="/reps/earnings">
            <Button variant="outline" className="w-full rounded-xl border-gray-200 dark:border-slate-700 hover:border-teal-300 hover:bg-teal-50/50 transition-all h-12">
              <DollarSign size={18} className="mr-2 text-teal-600 dark:text-teal-400" />
              Earnings
            </Button>
          </Link>
          <Link href="/reps/tasks">
            <Button variant="outline" className="w-full rounded-xl border-gray-200 dark:border-slate-700 hover:border-teal-300 hover:bg-teal-50/50 transition-all h-12">
              <Target size={18} className="mr-2 text-teal-600 dark:text-teal-400" />
              Tasks
            </Button>
          </Link>
        </div>
