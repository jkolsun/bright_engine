...
export default function RepsLayout({
  ...
  return (
    <ThemeProvider>
      <RepsLayoutInner>{children}</RepsLayoutInner>
    </ThemeProvider>
  )
}

function RepsLayoutInner({
  children,
}: {
  children: React.ReactNode
}) {
  ...
  return (
    <div className={`flex h-screen ${resolvedTheme === 'dark' ? 'dark bg-background' : 'bg-mesh-teal'}`} style={resolvedTheme === 'dark' ? undefined : { background: 'linear-gradient(135deg, #f8fafc 0%, #f0fdfa 40%, #ecfdf5 70%, #f0f9ff 100%)' }}>
      ...
      {!isDialerPage && <nav className="fixed bottom-0 left-0 right-0 z-40 md:hidden bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-700 shadow-lg">
      ...
      {feedbackOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setFeedbackOpen(false)}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in" onClick={(e) => e.stopPropagation()}>
            {/* Dialog Header */}
            ...
            <div className="p-6">
              {feedbackSent ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-lg font-bold text-gray-900 dark:text-gray-100">Thanks for your feedback!</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">We&apos;ll review it shortly.</p>
                </div>
              ) : (
                <>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider block mb-1.5">Category</label>
                      <select
                        value={feedbackCategory}
                        onChange={(e) => setFeedbackCategory(e.target.value)}
                        className="w-full px-4 py-2.5 border border-gray-200 dark:border-slate-700 rounded-xl text-sm bg-gray-50/50 dark:bg-slate-800/50 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all"
                      >
                    ...
                    <div>
                      <label className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider block mb-1.5">Message</label>
                      <textarea
                        value={feedbackMessage}
                        onChange={(e) => setFeedbackMessage(e.target.value)}
                        placeholder="Type your feedback here..."
                        className="w-full h-32 px-4 py-3 border border-gray-200 dark:border-slate-700 rounded-xl text-sm bg-gray-50/50 dark:bg-slate-800/50 dark:text-gray-100 resize-none focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all"
                      />
                    </div>
                    <button
      ...
      {showWelcomeToast && (
        <div className="fixed top-6 right-6 z-50 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-green-200 dark:border-green-800 rounded-xl shadow-lg p-4 flex items-center gap-3 max-w-sm">
            <div className="w-10 h-10 rounded-full bg-green-100 dark:bg-green-900/40 flex items-center justify-center flex-shrink-0">
              <CheckCircle size={20} className="text-green-600 dark:text-green-400" />
            </div>
            <div>
              <p className="font-semibold text-gray-900 dark:text-gray-100 text-sm">You&apos;re all set!</p>
              <p className="text-gray-500 dark:text-gray-400 text-xs">Your leads are ready. Start dialing!</p>
            </div>
            <button onClick={() => setShowWelcomeToast(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 ml-2">
              <X size={16} />
            </button>
          </div>
        </div>
