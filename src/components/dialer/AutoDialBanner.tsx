...
export function AutoDialBanner() {
  ...
  if (autoDialBanner.type === 'dialing') {
    return (
      <div className="bg-blue-50 dark:bg-blue-950/30 border-b border-blue-200 px-4 py-2 flex items-center gap-2">
  ...
  if (autoDialBanner.type === 'chaining') {
    return (
      <div className="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-700 px-4 py-2 flex items-center gap-2">
    ...
    return (
      <button
        ...
        className={`w-full border-b px-4 py-2.5 flex items-center gap-2 transition-colors ${
          bannerUrgent
            ? 'bg-amber-100 dark:bg-amber-900/40 border-amber-400 animate-pulse'
            : 'bg-green-50 dark:bg-green-950/30 border-green-200'
        } hover:bg-green-200`}
      >
        <Phone className={`w-4 h-4 ${bannerUrgent ? 'text-amber-600' : 'text-green-600'}`} />
        <span className={`text-sm font-semibold ${bannerUrgent ? 'text-amber-900' : 'text-green-800 dark:text-green-400'}`}>
          {bannerUrgent ? 'WAITING: ' : 'CONNECTED: '}{autoDialBanner.leadName}
        </span>
        <span className={`text-xs ml-auto ${bannerUrgent ? 'text-amber-700 dark:text-amber-400 font-semibold' : 'text-green-600'}`}>
