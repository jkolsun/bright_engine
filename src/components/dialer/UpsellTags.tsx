  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-3">
      ...
      <div className="flex flex-wrap gap-1.5">
        {tags.map(tag => (
          <span key={tag.id} className="inline-flex items-center gap-1 px-2 py-1 bg-teal-50 dark:bg-teal-950/30 text-teal-700 dark:text-teal-400 rounded-md text-xs">
            {tag.productName} (${tag.productPrice})
            <button onClick={() => removeTag(tag.id)}><X className="w-3 h-3" /></button>
          </span>
        ))}
        {tags.length === 0 && !showPicker && <span className="text-xs text-gray-400 dark:text-gray-500">No upsells tagged</span>}
      </div>
      {showPicker && (
        <div className="mt-2 border border-gray-200 dark:border-slate-700 rounded-lg p-2 max-h-32 overflow-y-auto">
          {products.map(p => (
            <button key={p.id} onClick={() => addTag(p.id)} className="w-full text-left px-2 py-1.5 text-xs hover:bg-gray-50 dark:hover:bg-slate-800 rounded">
              {p.name} — ${p.price}
            </button>
          ))}
        </div>
