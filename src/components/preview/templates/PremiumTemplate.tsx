import { Phone, MapPin, Star, Shield, ArrowRight, Mail, CheckCircle, Target, Handshake, Scale, Briefcase } from 'lucide-react'
...
export default function PremiumTemplate({ lead, config, onCTAClick, onCallClick, websiteCopy }: TemplateProps) {
  ...
  return (
    <div className="preview-template min-h-screen bg-gray-950">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-28 md:py-40">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            ...
            <div>
              ...
              <p className="text-lg md:text-xl text-gray-300 leading-relaxed mb-6">{wc?.heroHeadline || config.tagline}</p>
            ...
            <div className="border border-amber-500/15 rounded-3xl p-10 md:p-12 bg-gray-950/60 backdrop-blur-md shadow-2xl shadow-amber-500/5">
              ...
              <div className="flex flex-col gap-4">
                <a
                  href={`tel:${lead.phone}`}
                  onClick={onCallClick}
                  className="inline-flex items-center justify-center gap-2.5 bg-gradient-to-r from-amber-500 to-amber-400 text-black px-8 py-4.5 rounded-xl font-semibold text-lg hover:from-amber-400 hover:to-amber-300 transition-all shadow-lg animate-cta-glow-amber"
                >
                  <Phone size={20} />
                  Call Now
                </a>
                <button
                  onClick={onCTAClick}
                  className="inline-flex items-center justify-center gap-2.5 border border-amber-500/30 text-amber-400 px-8 py-4.5 rounded-xl font-semibold text-lg hover:bg-amber-500 hover:text-black transition-all"
                >
                  {config.ctaText}
                  <ArrowRight size={18} />
                </button>
      ...
      {/* ─── Trust Indicators ─── */}
      <section className="py-16 px-4 sm:px-6 bg-gray-950 border-y border-amber-500/10">
        <div className="max-w-6xl mx-auto flex flex-wrap items-center justify-center gap-10 md:gap-20">
          {hasRating && (
            <>
              <div className="text-center">
                <p className="font-display text-4xl font-light text-gradient-amber">{lead.enrichedRating}</p>
                ...
                <p className="text-xs uppercase tracking-[0.2em] text-gray-500 font-medium">Rating</p>
              </div>
              <div className="hidden md:block w-px h-16 bg-amber-500/10" />
            </>
          )}
          {lead.enrichedReviews && (
            <>
              <div className="text-center">
                ...
                <p className="text-xs uppercase tracking-[0.2em] text-gray-500 font-medium mt-2">Reviews</p>
              </div>
              <div className="hidden md:block w-px h-16 bg-amber-500/10" />
            </>
          )}
          <div className="text-center">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-lg text-white font-light">Licensed & Insured</p>
            </div>
            <p className="text-xs uppercase tracking-[0.2em] text-gray-500 font-medium">Full Coverage</p>
          </div>
          <div className="hidden md:block w-px h-16 bg-amber-500/10" />
          {location && (
            <div className="text-center">
              <div className="flex items-center gap-2 mb-1">
                <MapPin size={18} className="text-amber-400/70" />
                <p className="text-lg text-white font-light">{location}</p>
              </div>
              <p className="text-xs uppercase tracking-[0.2em] text-gray-500 font-medium">Service Area</p>
            </div>
          )}
        </div>
      </section>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {services.slice(0, 6).map((service, i) => (
                <div key={i} className="group flex items-center gap-5 bg-gray-950/60 border border-gray-800/30 rounded-2xl p-7 card-lift card-lift-dark hover:border-amber-500/20">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-sm">{wc?.serviceDescriptions?.[service] || `Expert ${service.toLowerCase()} tailored to your requirements.`}</p>
                  </div>
                  <ArrowRight size={16} className="text-amber-500/0 group-hover:text-amber-500/60 transition-all ml-auto flex-shrink-0" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {[
              ...
              <div key={i} className="text-center group">
                <div className="w-20 h-20 rounded-2xl border border-amber-500/20 flex items-center justify-center mx-auto mb-8 group-hover:border-amber-500/40 group-hover:bg-amber-500/5 transition-all">
          ...
          <div className="lg:col-span-2">
            <div className="border border-amber-500/15 rounded-3xl p-10 bg-gray-950/60 backdrop-blur-sm sticky top-24">
              ...
              <div className="space-y-5">
                {lead.phone && (
                  <a href={`tel:${lead.phone}`} onClick={onCallClick} className="flex items-center gap-4 text-gray-400 hover:text-amber-400 transition-colors p-3 rounded-xl hover:bg-white/5">
                ...
                {lead.email && (
                  <a href={`mailto:${lead.email}`} className="flex items-center gap-4 text-gray-400 hover:text-amber-400 transition-colors p-3 rounded-xl hover:bg-white/5">
                    <div className="w-12 h-12 rounded-xl bg-amber-500/10 flex items-center justify-center flex-shrink-0">
              ...
              <button onClick={onCTAClick} className="w-full mt-10 bg-gradient-to-r from-amber-500 to-amber-400 text-black py-4.5 rounded-xl font-semibold text-lg hover:from-amber-400 hover:to-amber-300 transition-all shadow-lg">
                {config.ctaText}
              </button>
            </div>
          </div>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {photos.slice(0, 8).map((photo, i) => (
                <div key={i} className="group aspect-square bg-gray-900 rounded-2xl overflow-hidden border border-gray-800/50 hover:border-amber-500/20 transition-all duration-500 hover:shadow-xl hover:shadow-amber-500/5">
      ...
      <section className="py-24 px-4 sm:px-6 bg-gray-950 border-t border-amber-500/10 relative overflow-hidden">
        <div className="absolute inset-0 bg-mesh-dark" />
        <div className="absolute top-0 left-1/2 w-[500px] h-[500px] bg-amber-500/3 rounded-full -translate-y-1/2 -translate-x-1/2 blur-3xl" />
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href={`tel:${lead.phone}`}
              onClick={onCallClick}
              className="inline-flex items-center justify-center gap-2.5 bg-gradient-to-r from-amber-500 to-amber-400 text-black px-10 py-4.5 rounded-xl font-semibold text-lg hover:from-amber-400 hover:to-amber-300 transition-all shadow-lg animate-cta-glow-amber"
            >
              <Phone size={20} />
              {lead.phone || 'Call Now'}
            </a>
            <button
              onClick={onCTAClick}
              className="inline-flex items-center justify-center gap-2.5 border border-amber-500/30 text-amber-400 px-10 py-4.5 rounded-xl font-semibold text-lg hover:bg-amber-500 hover:text-black transition-all"
            >
              {config.ctaText}
              <ArrowRight size={18} />
            </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            ...
            <div>
              ...
              <div className="space-y-3 text-sm text-gray-500">
                {lead.phone && <p className="flex items-center gap-2.5 hover:text-white transition-colors"><Phone size={14} className="text-amber-500/40" /> {lead.phone}</p>}
                {lead.email && <p className="flex items-center gap-2.5 hover:text-white transition-colors"><Mail size={14} className="text-amber-500/40" /> {lead.email}</p>}
