  ...
  return (
    <>
      <button onClick={() => setIsOpen(!isOpen)} className="fixed bottom-6 right-5 z-[100] group sm:bottom-6 bottom-20" aria-label="Chat">
        <div className="w-[58px] h-[58px] rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 hover:scale-105 border border-amber-500/20" style={{ background: accentHex || 'linear-gradient(135deg, #f59e0b, #d97706)' }}>
      ...
      {isOpen && (
        <div className="fixed sm:bottom-24 bottom-28 right-5 z-[100] w-[370px] max-w-[calc(100vw-2.5rem)] bg-gray-900 rounded-2xl shadow-2xl border border-amber-500/15 overflow-hidden">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-[300px] bg-gray-900 shadow-2xl border-l border-amber-500/10">
        <div className="p-6 h-full flex flex-col">
          ...
          <div className="space-y-3">
            ...
            <button onClick={() => { onCTAClick(); onNavigate('contact'); onClose() }} className={`flex items-center justify-center gap-2 w-full py-3.5 rounded-xl text-black font-semibold text-sm ${config.primaryHex ? '' : ''}`} style={{ background: config.primaryHex ? `linear-gradient(135deg, ${config.primaryHex}, ${config.secondaryHex || config.primaryHex})` : 'linear-gradient(135deg, #f59e0b, #d97706)' }}>Free Consultation</button>
          </div>
        </div>
      </div>
    </div>
...
function CTABand({ closingHeadline, location, onCTAClick, onNavigateContact, config }: { closingHeadline?: string; location: string; onCTAClick: () => Promise<void>; onNavigateContact: () => void; config: import('../config/template-types').IndustryConfig }) {
  return (
    <section className="relative py-16 sm:py-20 md:py-24 overflow-hidden" style={{ background: config.primaryHex ? `linear-gradient(135deg, ${config.primaryHex}, ${config.secondaryHex || config.primaryHex})` : 'linear-gradient(135deg, #d97706, #b45309)' }}>
    ...
    <div className="preview-template min-h-screen bg-gray-950 antialiased">
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="flex items-center justify-between h-[68px]">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => { onCTAClick(); navigateTo('contact') }} className="hidden sm:flex text-black px-5 py-2.5 rounded-lg text-sm font-semibold transition-all shadow-md" style={{ background: config.primaryHex ? `linear-gradient(135deg, ${config.primaryHex}, ${config.secondaryHex || config.primaryHex})` : 'linear-gradient(135deg, #f59e0b, #d97706)' }}>Free Consultation</button>
      ...
      <PageShell page="services" currentPage={currentPage}>
        <PageHeader
          ...
          bgClass={config.primaryHex ? '' : 'bg-gradient-to-r from-amber-700 via-amber-600 to-yellow-600'}
