      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white/95 backdrop-blur-xl shadow-sm border-b border-green-100' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="flex items-center justify-between h-[68px]">
            ...
            <div className="hidden lg:flex items-center gap-1.5">{navSections.map((s) => (<a key={s.id} href={`#${s.id}`} className={`px-3.5 py-2 rounded-lg text-sm font-medium transition-all ${scrolled ? 'text-green-700/60 hover:text-green-900 hover:bg-green-50' : 'text-white/70 hover:text-white hover:bg-white/10'}`}>{s.label}</a>))}</div>
            <div className="flex items-center gap-3">
              ...
              {lead.phone && (<a href={`tel:${lead.phone}`} onClick={onCallClick} className={`hidden lg:flex items-center gap-2 text-sm font-medium transition-colors ${scrolled ? 'text-green-700/60 hover:text-green-900' : 'text-white/70 hover:text-white'}`}><Phone size={14} />{formatNavPhone(lead.phone)}</a>)}
              ...
              return (
                <div className={`grid grid-cols-1 ${testimonials.length === 1 ? 'max-w-2xl mx-auto' : 'md:grid-cols-2'} gap-6`}>
                  {testimonials.map((r, i) => (
                    <ScrollReveal key={i} animation="fade-up" delay={i * 100}>
                    <div className="bg-green-50/50 border border-green-100 rounded-2xl p-7 sm:p-8 hover:border-green-200 transition-all">
