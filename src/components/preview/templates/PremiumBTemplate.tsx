      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-stone-50/95 backdrop-blur-xl border-b border-stone-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="flex items-center justify-between h-[68px]">
            ...
            <div className="hidden lg:flex items-center gap-1.5">{navSections.map((s) => (<a key={s.id} href={`#${s.id}`} className="px-3.5 py-2 rounded-lg text-sm font-medium text-stone-500 hover:text-emerald-700 hover:bg-emerald-50 transition-all">{s.label}</a>))}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {(() => {
              ...
              return testimonials.map((r, i) => (
                ...
                <div className={`bg-white border border-stone-200 rounded-2xl p-8 hover:border-emerald-400/30 transition-all ${i === 2 ? 'md:max-w-lg md:mx-auto' : ''}`}>
