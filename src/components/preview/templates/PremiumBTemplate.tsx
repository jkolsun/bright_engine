 ...
 * Removed: service card grid, "Our Approach" 3-step
 * Services → numbered list, gallery → asymmetric, testimonial → 3 staggered
 * Value props folded into About section
 */
    ...
    <div className="fixed inset-0 z-[90] lg:hidden">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-[300px] bg-white shadow-2xl">
        <div className="p-6 h-full flex flex-col">
          ...
          <nav className="space-y-1 flex-1">{sections.map((s) => (<a key={s.id} href={`#${s.id}`} onClick={onClose} className="flex items-center justify-between px-4 py-3.5 rounded-xl text-stone-700 hover:bg-emerald-50 transition-all text-[15px] font-medium">{s.label}<ChevronRight size={16} className="text-stone-300" /></a>))}</nav>
          ...
          <div className="space-y-3">
            {phone && (<a href={`tel:${phone}`} onClick={() => { onCallClick(); onClose() }} className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl bg-stone-100 text-stone-800 font-semibold text-sm border border-stone-200"><Phone size={16} />Call {phone}</a>)}
...
export default function PremiumBTemplate({ lead, config, onCTAClick, onCallClick, websiteCopy }: TemplateProps) {
  ...
  const faqs = [
    { q: 'How does the consultation work?', a: 'We begin with a thorough assessment of your needs, then provide a detailed, transparent estimate — all at no cost or obligation.' },
    { q: 'What areas do you serve?', a: `We serve ${location || 'the local area'} and surrounding communities. Please reach out to confirm coverage in your area.` },
    ...
    { q: 'How soon can work begin?', a: 'Most projects commence within 1–2 weeks. We offer priority scheduling for time-sensitive needs.' },
    { q: 'What guarantees do you provide?', a: 'Every project is backed by our satisfaction guarantee. We stand behind our work — completely.' },
  ]
    ...
    <div className="preview-template min-h-screen bg-stone-50 antialiased">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-stone-50/95 backdrop-blur-xl border-b border-stone-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="flex items-center justify-between h-[68px]">
            ...
            <div className="hidden lg:flex items-center gap-1">{navSections.map((s) => (<a key={s.id} href={`#${s.id}`} className="px-3 py-2 rounded-lg text-[13px] font-medium text-stone-500 hover:text-emerald-700 hover:bg-emerald-50 transition-all">{s.label}</a>))}</div>
            <div className="flex items-center gap-3">
              ...
              {lead.phone && (<a href={`tel:${lead.phone}`} onClick={onCallClick} className="hidden lg:flex items-center gap-2 text-sm text-stone-500 hover:text-emerald-700 font-medium"><Phone size={14} />{lead.phone}</a>)}
      ...
      <section id="hero" className="relative min-h-[100svh] flex items-center justify-center overflow-hidden bg-stone-50">
        ...
        <div className="relative max-w-4xl mx-auto w-full px-5 sm:px-8 py-32 text-center">
          <p className="text-xs uppercase tracking-[0.3em] text-emerald-600/50 mb-6 font-medium">Premium {industryLabel}{location ? ` · ${location}` : ''}</p>
          ...
          <p className="text-lg sm:text-xl text-stone-500 leading-relaxed max-w-2xl mx-auto mb-4">{wc?.heroHeadline || config.tagline}</p>
          {wc?.heroSubheadline && <p className="text-base text-stone-400 leading-relaxed max-w-xl mx-auto mb-8">{wc.heroSubheadline}</p>}
          {hasRating && (
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            {lead.phone && (<a href={`tel:${lead.phone}`} onClick={onCallClick} className="inline-flex items-center justify-center gap-2.5 bg-emerald-700 text-white px-10 py-4 rounded-xl font-semibold text-lg hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-700/20"><Phone size={20} />Call Now — Free Consultation</a>)}
      ...
      <section className="relative py-20 sm:py-24 overflow-hidden bg-emerald-800">
        ...
        <div className="relative max-w-4xl mx-auto px-5 sm:px-8 text-center">
          ...
          {location && <p className="mt-5 text-emerald-200/40 text-sm font-medium tracking-wide">Serving {location}</p>}
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 lg:gap-16 items-start">
            ...
            <div className="lg:col-span-2 flex flex-col gap-5">
              <div className="bg-white border border-stone-200 rounded-2xl p-7">
                ...
                <p className="text-stone-600 text-base italic leading-relaxed mb-4">"Working with {lead.companyName} was exceptional. Their attention to detail and professionalism truly set them apart."</p>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
              {photos.slice(0, 6).map((photo, i) => (
                <div key={i} className={`relative overflow-hidden rounded-2xl group border border-stone-200 hover:border-emerald-400/40 transition-all ${i === 0 ? 'md:col-span-2 md:row-span-2' : ''}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              ...
              <div key={i} className={`bg-white border border-stone-200 rounded-2xl p-8 hover:border-emerald-400/30 transition-all ${i === 2 ? 'md:col-span-2 md:max-w-lg md:mx-auto' : ''}`}>
                <div className="flex gap-0.5 mb-5">{Array.from({ length: 5 }, (_, j) => <Star key={j} size={15} className="text-emerald-500 fill-current" />)}</div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <div>
              ...
              <p className="text-stone-500 text-base leading-relaxed mb-10">{wc?.closingBody || `Schedule a complimentary consultation with ${lead.companyName}. We look forward to exceeding your expectations.`}</p>
      ...
      <footer className="bg-emerald-900 py-14 px-5 sm:px-8 border-t border-emerald-700/50">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
            ...
            <div><h4 className="text-xs uppercase tracking-[0.2em] text-emerald-400/40 mb-4 font-medium">Services</h4><ul className="space-y-2.5 text-sm text-emerald-100/30">{services.slice(0, 5).map((s, i) => <li key={i} className="flex items-center gap-2 hover:text-white transition-colors cursor-pointer"><CheckCircle size={11} className="text-emerald-500/40 flex-shrink-0" />{s}</li>)}</ul></div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 sm:hidden bg-white/95 backdrop-blur-xl border-t border-stone-200 px-4 py-3">
        <div className="flex gap-3">
          ...
          <button onClick={onCTAClick} className="flex-[2] flex items-center justify-center gap-2 py-3 rounded-xl bg-emerald-700 text-white font-semibold text-sm">Free Consultation</button>
        </div>
      </div>
