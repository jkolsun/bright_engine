  ...
  return (
    <>
      <button onClick={() => setIsOpen(!isOpen)} className="fixed bottom-6 right-5 z-[100] group sm:bottom-6 bottom-20" aria-label="Chat">
        <div className={`w-[58px] h-[58px] rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 hover:scale-105 ${accentHex ? '' : 'bg-emerald-700'} border border-emerald-600`} style={accentHex ? { background: accentHex } : undefined}>
      ...
      {isOpen && (
        <div className="fixed sm:bottom-24 bottom-28 right-5 z-[100] w-[370px] max-w-[calc(100vw-2.5rem)] bg-white rounded-2xl shadow-2xl border border-stone-200 overflow-hidden">
          ...
          <div className="px-4 py-3 border-t border-stone-100 bg-white"><div className="flex gap-2"><input ref={inputRef} type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Type a message..." className="flex-1 text-sm px-4 py-2.5 rounded-full bg-stone-50 border border-stone-200 text-stone-800 focus:outline-none focus:ring-2 focus:ring-emerald-400/30 placeholder:text-stone-400" /><button onClick={() => handleSend()} disabled={!input.trim()} className={`w-10 h-10 rounded-full flex items-center justify-center text-white transition-all disabled:opacity-30 ${accentHex ? '' : 'bg-emerald-700'}`} style={accentHex ? { background: accentHex } : undefined}><Send size={15} /></button></div></div>
        </div>
      )}
    </>
  )
      ...
      <div className="absolute right-0 top-0 bottom-0 w-[300px] bg-white shadow-2xl">
        <div className="p-6 h-full flex flex-col">
          ...
          <div className="flex gap-3 mb-5">
            <a href="#" onClick={(e) => e.preventDefault()} className="w-10 h-10 rounded-lg bg-stone-100 flex items-center justify-center text-stone-500 hover:text-emerald-700 transition-colors"><Facebook size={16} /></a>
            <a href="#" onClick={(e) => e.preventDefault()} className="w-10 h-10 rounded-lg bg-stone-100 flex items-center justify-center text-stone-500 hover:text-emerald-700 transition-colors"><Instagram size={16} /></a>
            <a href="#" onClick={(e) => e.preventDefault()} className="w-10 h-10 rounded-lg bg-stone-100 flex items-center justify-center text-stone-500 hover:text-emerald-700 transition-colors"><GoogleIcon size={16} /></a>
          </div>
          <div className="space-y-3">
            ...
            <button onClick={() => { onCTAClick(); onNavigate('contact'); onClose() }} className={`flex items-center justify-center gap-2 w-full py-3.5 rounded-xl text-white font-semibold text-sm ${config.primaryHex ? '' : 'bg-emerald-700'}`} style={brandGradientStyle(config, 'to right') || undefined}>Free Consultation</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="preview-template min-h-screen bg-stone-50 antialiased">
          ...
          <div className="flex items-center justify-between h-[68px]">
            ...
            <div className="flex items-center gap-3">
              <div className="hidden md:flex items-center gap-0.5 text-stone-400">
                <a href="#" onClick={(e) => e.preventDefault()} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-emerald-50 hover:text-emerald-600 transition-all"><Facebook size={14} /></a>
                <a href="#" onClick={(e) => e.preventDefault()} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-emerald-50 hover:text-emerald-600 transition-all"><Instagram size={14} /></a>
                <a href="#" onClick={(e) => e.preventDefault()} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-emerald-50 hover:text-emerald-600 transition-all"><GoogleIcon size={13} /></a>
              </div>
              ...
              <button onClick={() => { onCTAClick(); navigateTo('contact') }} className={`hidden sm:flex text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:opacity-90 transition-all shadow-md ${config.primaryHex ? '' : 'bg-emerald-700 hover:bg-emerald-600'}`} style={brandGradientStyle(config, 'to right') || undefined}>Free Consultation</button>
      ...
      <PageShell page="services" currentPage={currentPage}>
        <PageHeader
          ...
          bgClass={config.primaryHex ? '' : 'bg-emerald-800'}
            ...
            {photos.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
                {photos.slice(0, 6).map((photo, i) => (
                  ...
                  <div className="relative overflow-hidden rounded-2xl group border border-stone-200 hover:border-emerald-400/40 transition-all cursor-pointer" onClick={() => { setLightboxIndex(i); setLightboxOpen(true) }}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
              ...
              <div>
                ...
                <div className="flex gap-3 mt-10">
                  <a href="#" onClick={(e) => e.preventDefault()} className="w-10 h-10 rounded-xl bg-stone-50 border border-stone-200 flex items-center justify-center text-stone-400 hover:text-emerald-600 hover:border-emerald-200 transition-all"><Facebook size={16} /></a>
                  <a href="#" onClick={(e) => e.preventDefault()} className="w-10 h-10 rounded-xl bg-stone-50 border border-stone-200 flex items-center justify-center text-stone-400 hover:text-emerald-600 hover:border-emerald-200 transition-all"><Instagram size={16} /></a>
                  <a href="#" onClick={(e) => e.preventDefault()} className="w-10 h-10 rounded-xl bg-stone-50 border border-stone-200 flex items-center justify-center text-stone-400 hover:text-emerald-600 hover:border-emerald-200 transition-all"><GoogleIcon size={16} /></a>
                </div>
              </div>
              </ScrollReveal>
              <ScrollReveal animation="fade-right" delay={200}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
            <div>
              ...
              <div className="flex gap-2.5 mt-5">
                <a href="#" onClick={(e) => e.preventDefault()} className="w-8 h-8 rounded-lg bg-emerald-800 border border-emerald-700/50 flex items-center justify-center text-emerald-100/30 hover:text-white transition-all"><Facebook size={13} /></a>
                <a href="#" onClick={(e) => e.preventDefault()} className="w-8 h-8 rounded-lg bg-emerald-800 border border-emerald-700/50 flex items-center justify-center text-emerald-100/30 hover:text-white transition-all"><Instagram size={13} /></a>
                <a href="#" onClick={(e) => e.preventDefault()} className="w-8 h-8 rounded-lg bg-emerald-800 border border-emerald-700/50 flex items-center justify-center text-emerald-100/30 hover:text-white transition-all"><GoogleIcon size={12} /></a>
              </div>
            </div>
            <div>
